...
data.pheno <- as.data.frame(pData(data[[1]])[, c(2,54,53,54)])
names(data.pheno)[1]<-"#Identifier"
names(data.pheno)[2]<-"@Class"
names(data.pheno)[3]<-"Class1"
names(data.pheno)[4]<-"Class2"

paste_noNA <- function(x,sep=", ") {
        gsub(", " ,sep, toString(x[!is.na(x) & x!="" & x!="NA"] ) ) }
data.pheno$"@Class" <- apply(data.pheno[ , c("Class1","Class2") ] , 1 , paste_noNA , sep="")

data.pheno <- subset(data.pheno, select = -c(Class1, Class2))


# Edit outcome values to Case/Normal
View(data.pheno)
data.pheno$"@Class" <- gsub('.*Disease', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('normal', 'Normal', data.pheno$"@Class")
