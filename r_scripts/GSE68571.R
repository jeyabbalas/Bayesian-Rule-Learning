...
get_annotation_db("GPL80")

biocLite("hu6800.db")
library(hu6800.db)

probe_ids = rownames(data.exp)
gene_symbols = unlist(mget(probe_ids, hu6800SYMBOL, ifnotfound=NA))
...
View(data.pheno)
data.pheno$"@Class" <- gsub('lung cancer', 'Case', data.pheno$"@Class")
