...
data.pheno <- as.data.frame(pData(data[[1]])[, c(2,10)])
names(data.pheno)[1]<-"#Identifier"
names(data.pheno)[2]<-"@Class"

# Edit outcome values to Case/Normal
View(data.pheno)
data.pheno$"@Class" <- gsub('Parkinson\'s disease', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('healthy control', 'Normal', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('neurological disease control', 'Normal', data.pheno$"@Class")
