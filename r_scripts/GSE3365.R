...
get_annotation_db("GPL96")

biocLite("hgu133a.db")
library(hgu133a.db)

probe_ids = rownames(data.exp)
gene_symbols = unlist(mget(probe_ids, hgu133aSYMBOL, ifnotfound=NA))
...
data.pheno <- as.data.frame(pData(data[[1]])[, c(2,13)])
names(data.pheno)[1]<-"#Identifier"
names(data.pheno)[2]<-"@Class"

# Edit outcome values to Case/Normal
View(data.pheno)
data.pheno$"@Class" <- gsub('Subject Disease : Crohn\'s Disease', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('Subject Disease : Ulcerative Colitis', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('Subject Disease : Normal', 'Normal', data.pheno$"@Class")
