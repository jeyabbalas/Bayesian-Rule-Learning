...
data.pheno <- as.data.frame(pData(data[[1]])[, c(2,25,1)])
...
View(data.pheno)
data.pheno$"@Class" <- gsub('.*cognitively intact.*', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('post-mortem tissue', 'Normal', data.pheno$"@Class")

data.pheno$"Group" <- gsub('^post-central.*', 'Select', data.pheno$"Group")
data.pheno$"Group" <- gsub('^PostcentralGyrus.*', 'Select', data.pheno$"Group")

# Merge expression data with phenotype
data.all <- cbind(data.pheno, logd)

# Only keep instances measured before treatment.
data.all <- data.all[grep("Select",data.pheno$Group),]
