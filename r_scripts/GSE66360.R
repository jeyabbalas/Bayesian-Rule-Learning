...
# Finding the annotation database for a given GPL platform id.
library(GEOmetadb)
...
get_annotation_db <- function(gpl_id) {
        query = paste("select gpl,title,bioc_package from gpl 
                         where gpl='", gpl_id, "'", sep="")
        res = dbGetQuery(con,query)
        return(res$bioc_package)
}

############ Extracting dataset 1: GSE66360 ############
geo_id = 'GSE66360'
...
View(data.pheno)
data.pheno$"@Class" <- gsub('Myocardial Infarction', 'Case', data.pheno$"@Class")
