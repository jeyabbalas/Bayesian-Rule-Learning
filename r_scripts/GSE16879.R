...
data.pheno <- as.data.frame(pData(data[[1]])[, c(2,37,36)])
...
View(data.pheno)
data.pheno$"@Class" <- gsub('UC', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('CD', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('Control', 'Normal', data.pheno$"@Class")

data.pheno$"Group" <- gsub('Not applicable', 'Before', data.pheno$"Group")
data.pheno$"Group" <- gsub('Before first infliximab treatment', 'Before', data.pheno$"Group")
data.pheno$"Group" <- gsub('After first infliximab treatment', 'After', data.pheno$"Group")

# Merge expression data with phenotype
data.all <- cbind(data.pheno, logd)

# Only keep instances measured before treatment.
data.all <- data.all[grep("^Before",data.pheno$Group),]
