...
data.pheno <- as.data.frame(pData(data[[1]])[, c(2,37)])
names(data.pheno)[1]<-"#Identifier"
names(data.pheno)[2]<-"@Class"

# Edit outcome values to Case/Normal
View(data.pheno)
data.pheno$"@Class" <- gsub('Definite MS', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('CIS', 'Normal', data.pheno$"@Class")
