...
############ Extracting dataset 12: GSE55447 ############
geo_id = 'GSE55447'
...
get_annotation_db("GPL10558")

biocLite("illuminaHumanv4.db")
library(illuminaHumanv4.db)

probe_ids = rownames(data.exp)
gene_symbols = unlist(mget(probe_ids, illuminaHumanv4SYMBOL, ifnotfound=NA))
...
data.pheno <- as.data.frame(pData(data[[1]])[, c(2,1,1)])
...
View(data.pheno)
data.pheno$"@Class" <- gsub('.*Case.*', 'Case', data.pheno$"@Class")
data.pheno$"@Class" <- gsub('.*Control.*', 'Normal', data.pheno$"@Class")

data.pheno$"Group" <- gsub('.*CD4', 'Select', data.pheno$"Group")
